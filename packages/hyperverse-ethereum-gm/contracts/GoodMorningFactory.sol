...
import './GoodMorning.sol';

/**
 * @dev Clone Factory Implementation for ERC20 Token
 */

contract GoodMorningFactory is CloneFactory {
	struct Tenant {
		GoodMorning module;
		address owner;
	}
	...
	function createInstance(address _tenant) external isAllowedToCreateInstance(_tenant) {
		GoodMorning m = GoodMorning(createClone(masterContract));
	...
	function getProxy(address _tenant) public view returns (GoodMorning) {
